...
# python generate_descriptions_Weather.py --csv_path Dataset\jena_climate_2009_2016.csv --target_col "T (degC)" --max_samples 5
# python generate_descriptions_Weather.py --csv_path Dataset\jena_climate_2009_2016.csv --target_cols "T (degC)" "p (mbar)" --max_samples 5
# --target_col 只允许指定一列（默认 "T (degC)"）：会自动检测并加入强相关列
...
def load_weather(csv_path: str) -> pd.DataFrame:
    ...
    # jena_climate 的时间格式类似 '01.01.2009 00:10:00'（日.月.年），使用 dayfirst=True 更稳健
    try:
    ...
    if not numeric_cols:
        # 尝试将非时间列强制转换为数值（常见于jena_climate）
        ...
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    if not numeric_cols:
        raise ValueError("未能识别出任何数值列，请检查数据集格式。")
...
# 常见气象量中文映射（匹配 jena_climate_2009_2016.csv 的列名）
DEFAULT_CHINESE_MAP = {
    'Date Time': '日期 时间',
    'p (mbar)': '气压(mbar)',
    'T (degC)': '气温(°C)',
    ...
    'Tdew (degC)': '露点温度(°C)',
    'rh (%)': '相对湿度(%)',
    'VPmax (mbar)': '最大蒸汽压(mbar)',
    'VPact (mbar)': '实际蒸汽压(mbar)',
    'VPdef (mbar)': '蒸汽压亏缺(mbar)',
    'sh (g/kg)': '比湿(g/kg)',
    'H2OC (mmol/mol)': '水汽浓度(mmol/mol)',
    'rho (g/m**3)': '空气密度(g/m**3)',
    'wv (m/s)': '风速(m/s)',
    'max. wv (m/s)': '最大风速(m/s)',
    'wd (deg)': '风向(度)'
}
...
def describe_window_weather(
    win_df: pd.DataFrame,
    main_target_col: str,
    target_cols: list[str] | None = None,
) -> tuple[str, dict]:
    target_cols = target_cols or [main_target_col]
    chinese_name = DEFAULT_CHINESE_MAP.get(main_target_col, main_target_col)
    target = win_df[main_target_col].to_numpy(dtype=float)
    L = len(target)
    # 判断索引是否为时间戳，以便更友好地展示时间范围
    ...
    for var in [c for c in win_df.columns if c != main_target_col]:
        var_chinese = DEFAULT_CHINESE_MAP.get(var, var)
        ...
        if rho_val is not None:
            corr_info.append({"var": var, "var_chinese": var_chinese, "rho": float(rho_val), "stable": bool(corr_stable_map[var])})
    ...
    lines.append(
        ...
        + f"{var_name} 的数值大致分布在 {g_min:.2f} 到 {g_max:.2f} 之间，平均约 {g_mean:.2f}，起始点为 {start_val:.2f}，结束点为 {end_val:.2f}，{trait}。"
    )
    ...
    if events:
        ...
        for ev in events[:2]:
            s, e = ev['start'], ev['end']
            v, z = ev['peak_value'], ev['z_max']
            if has_timestamp:
                ev_start = win_df.index[s].strftime('%Y-%m-%d %H:%M')
                ev_end = win_df.index[e].strftime('%Y-%m-%d %H:%M')
                ev_time_desc = f"{ev_start} 到 {ev_end}"
            else:
                ev_time_desc = f"第 {s} 到 {e} 点"
            prev_idx = max(0, s - 5)
            ...
            event_desc.append(
                f"{ev_time_desc}，{var_name} 出现一次局部{change_word}（峰值约 {v:.2f}，相较此前一小段时间均值 {prev_mean:.2f} 明显{compare_word}，标准化偏离 |z|≈{abs(z):.2f}）"
            )
        lines.append("异常点：\n" + "；\n".join(event_desc) + "。")
    else:
        ...
        if extra_targets:
            ...
            for var in extra_targets:
                ...
                if rho is None or not stable:
                    parts.append(f"{var_chinese} 与 {var_name} 无明显联动关系")
                elif abs(rho) >= 0.25:
                    if rho >= 0:
                        parts.append(f"{var_chinese} 与 {var_name} 呈较强正相关（相关系数约 {rho:.2f}）")
                    else:
                        parts.append(f"{var_chinese} 与 {var_name} 呈较强负相关（相关系数约 {rho:.2f}）")
                else:
                    parts.append(f"{var_chinese} 与 {var_name} 联动关系弱（相关系数约 {rho:.2f}）")
            lines.append("联动关系：\n" + "；\n".join(parts) + "。")
...
def _parse_target_cols(target_cols_input: list[str] | str | None, default_col: str = 'T (degC)') -> list[str]:
...
def generate_weather_jsonl(
    csv_path: str,
    output_path: str,
    target_col: str = 'T (degC)',
    target_cols: list[str] | str | None = None,
    window_lengths=(144, 288),
    step_ratio: float = 0.5,
    max_samples: int | None = None,
):
    df = load_weather(csv_path)
    ...
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"目标列 {col} 不存在，数据集中可用列：{list(df.columns)[:10]}...")
    ...
    with out_path.open('w', encoding='utf-8') as f:
        for L in window_lengths:
            ...
            for start in sliding_window_indices(n, L, step):
                ...
                feature_names_chinese = [DEFAULT_CHINESE_MAP.get(c, c) for c in sample_keep_cols]
                sample_strong_corr_cols = [c for c in sample_keep_cols if c != main_target_col]
                desc, feat = describe_window_weather(
                    win_df,
                    main_target_col=main_target_col,
                    target_cols=sample_keep_cols,
                )
                sample = {
                    'id': f"Weather_{main_target_col}_L{L}_start{start}",
                    'dataset': 'Weather',
                    'task': 'forecasting',
                    'window_length': L,
                    ...
                    'target_cols': sample_keep_cols,
                    'target_col_chinese': DEFAULT_CHINESE_MAP.get(main_target_col, main_target_col),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='生成Weather数据集的中文文本描述（JSONL格式）。')
    parser.add_argument('--csv_path', type=str, required=True, help='输入CSV路径')
    parser.add_argument('--output_path', type=str, default=None, help='输出jsonl路径（默认为脚本目录，根据目标列自动命名）')
    parser.add_argument('--target_col', type=str, default='T (degC)', help='主目标列（仅单列，默认 "T (degC)"）。不指定 --target_cols 时，会自动补充强相关列')
    parser.add_argument('--target_cols', type=str, nargs='+', default=None, help='显式指定要写入 jsonl 的列（支持一列或多列，空格或逗号分隔）')
    parser.add_argument('--window_lengths', type=int, nargs='+', default=[144, 288], help='窗口长度列表（默认144/288）')
    parser.add_argument('--step_ratio', type=float, default=0.5, help='滑动步长与窗口长度的比例')
    parser.add_argument('--max_samples', type=int, default=None, help='最多生成多少个样本')
    ...
    if args.output_path is None:
        ...
        # 将目标列变为文件名安全的形式（替换 Windows/类 Unix 不允许的字符）
        safe_col = re.sub(r'[<>:"/\\|?*]', '_', main_target_col)
        win_tag = "-".join(str(w) for w in args.window_lengths)
        default_name = f"Weather_{safe_col}_L{win_tag}_descriptions.jsonl"
        output_path = script_dir / default_name
    else:
        output_path = Path(args.output_path)

    generate_weather_jsonl(
