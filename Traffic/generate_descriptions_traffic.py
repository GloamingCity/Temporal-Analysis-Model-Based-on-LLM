...
# python generate_descriptions_traffic.py --csv_path Metro_Interstate_Traffic_Volume.csv --max_samples 5
# python generate_descriptions_traffic.py --csv_path Metro_Interstate_Traffic_Volume.csv --target_col traffic_volume --max_samples 5
# python generate_descriptions_traffic.py --csv_path Metro_Interstate_Traffic_Volume.csv --target_cols traffic_volume temp --max_samples 5
# 添加“ --max_samples x”生成少量数据，x表示生成x行jsonl数据
# 数据集包含交通流量（traffic_volume）以及天气相关特征（温度、降雨、降雪、云量等）
# --target_col 只允许指定一列（默认 traffic_volume）：会自动检测并加入强相关列
...
def load_traffic(csv_path: str) -> pd.DataFrame:
    ...
    if time_col not in df.columns:
        raise ValueError("数据集缺少 date_time 列")
    ...
    if target_col not in df.columns:
        raise ValueError("数据集缺少 traffic_volume 列")
...
def segment_features(x: np.ndarray, num_segments: int = 4, global_std: float = 0.0):
    """
    将序列分成 num_segments 段，提取每段的均值、标准差、波动等级、趋势标签。
    """
    x = np.asarray(x, dtype=float)
    L = len(x)
    if L < num_segments:
...
def describe_window_traffic(
    win_df: pd.DataFrame,
    main_target_col: str = "traffic_volume",
    target_cols: list[str] | None = None,
) -> tuple[str, dict]:
    """
    对Traffic的一个窗口生成中文描述和结构化特征
    :param win_df: 窗口DataFrame（含traffic_volume等列）
    :return: (描述文本, 结构化特征)
    """
    ...
    period_profile = evaluate_global_periodicity(target, best_lag, best_corr)
    periodic_cycle_flag = bool(period_profile.get("is_global_periodic", False) and not np.allclose(target, target[0]))
    ...
    corr_info = []
    corr_candidates = [
        c for c in win_df.select_dtypes(include=[np.number]).columns
        if c != main_target_col
    ]
    corr_map = {}
    corr_stable_map = {}
    ...
    # 天气相关性（如果有数据）
    weather_info = []
    if 'temp' in win_df.columns:
        temp_mean = float(win_df['temp'].mean())
        weather_info.append(f"平均温度约为 {temp_mean:.1f}K")
    if 'rain_1h' in win_df.columns:
        rain_sum = float(win_df['rain_1h'].sum())
        if rain_sum > 0:
            weather_info.append(f"累计降雨量约为 {rain_sum:.1f}mm")
    if 'snow_1h' in win_df.columns:
        snow_sum = float(win_df['snow_1h'].sum())
        if snow_sum > 0:
            weather_info.append(f"累计降雪量约为 {snow_sum:.1f}mm")
    weather_str = "，".join(weather_info) if weather_info else "天气条件未特别记录"
    ...
    lines = []
    var_name = "交通流量" if main_target_col == "traffic_volume" else str(main_target_col)
    ...
    lines.append(
        "窗口概览：\n"
        + f"这段窗口覆盖时间为 {t0} 到 {t1}，共 {L} 个观测点。"
        + f"{var_name} 的数值大致分布在 {g_min:.0f} 到 {g_max:.0f} 之间，平均约 {g_mean:.0f}，起始点为 {start_val:.0f}，结束点为 {end_val:.0f}，{trait}。"
        + f"{weather_str}。"
    )
    ...
    if segments:
        ...
        if no_change_local:
            ...
            periodic_shape_text = (
                ...
                f"高位段占比约 {high_pct}%，单周期取值大致在 {fmt_num(float(period_representative_bounds(target, int(best_lag))[0]))} 到 {fmt_num(float(period_representative_bounds(target, int(best_lag))[1]))} 之间"
            )
            segment_lines = [
    ...
    if events:
        ...
        for ev in events[:2]:
            ...
            v = ev["peak_value"]
            z = ev["z_max"]
            ev_start = win_df.index[s].strftime('%Y-%m-%d %H:%M')
            ev_end = win_df.index[e].strftime('%Y-%m-%d %H:%M')
            prev_idx = max(0, s - 5)
            ...
            event_desc.append(
                f"{ev_start} 到 {ev_end}，{var_name} 出现一次局部{change_word}（峰值约 {v:.0f}，相较此前一小段时间均值 {prev_mean:.0f} 明显{compare_word}，标准化偏离 |z|≈{abs(z):.1f}）"
            )
        lines.append("异常点：\n" + "；\n".join(event_desc) + "。")
    else:
        ...
        if extra_targets:
            ...
            for var in extra_targets:
                ...
                if rho is None or not stable:
                    parts.append(f"{var} 与 {var_name} 无明显联动关系")
                elif abs(rho) >= 0.25:
                    relation = "同向" if rho >= 0 else "反向"
                    parts.append(f"{var} 与 {var_name} 呈{relation}关系（相关系数 rho≈{rho:.2f}）")
                else:
                    parts.append(f"{var} 与 {var_name} 联动关系弱（相关系数 rho≈{rho:.2f}）")
            lines.append("联动关系：\n" + "；\n".join(parts) + "。")
...
def _parse_target_cols(target_cols_input: list[str] | str | None, default_col: str = "traffic_volume") -> list[str]:
        ...
        for L in window_lengths:
            ...
            for start in sliding_window_indices(n, L, step):
                ...
                if explicit_targets is not None:
                    ...
                    best_fallback_col = None
                    best_fallback_rho = 0.0
                    for col in corr_candidates:
                        ...
                        # 回退策略：若强联动列未命中，保留一个相关度最高且非平直的候选列，便于联动关系质检可视化。
                        if (not flat) and rho_val is not None and np.isfinite(rho_val):
                            if abs(rho_val) > abs(best_fallback_rho):
                                best_fallback_rho = float(rho_val)
                                best_fallback_col = col
                    if len(sample_keep_cols) == 1 and best_fallback_col is not None and abs(best_fallback_rho) >= 0.18:
                        sample_keep_cols.append(best_fallback_col)

                feature_names_chinese = []
                for col in sample_keep_cols:
                    if col == 'traffic_volume':
                        feature_names_chinese.append("交通流量")
                    elif col == 'temp':
                        feature_names_chinese.append("温度")
                    elif col == 'rain_1h':
                        feature_names_chinese.append("小时降雨量")
                    elif col == 'snow_1h':
                        feature_names_chinese.append("小时降雪量")
                    elif col == 'clouds_all':
                        feature_names_chinese.append("云量")
                    elif col == 'holiday':
                        feature_names_chinese.append("节假日")
                    elif col == 'weather_main':
                        feature_names_chinese.append("主要天气")
                    elif col == 'weather_description':
                        feature_names_chinese.append("天气描述")
                    else:
                        feature_names_chinese.append(col)

                sample_strong_corr_cols = [c for c in sample_keep_cols if c != main_target_col]
                desc, feat = describe_window_traffic(
                    win_df,
                    main_target_col=main_target_col,
                    target_cols=sample_keep_cols,
                )
                sample = {
                    "id": f"Traffic_{main_target_col}_L{L}_start{start}",
                    "dataset": "Traffic",
                    "task": "forecasting",
                    "window_length": L,
                    ...
                    "feature_names_chinese": feature_names_chinese,
                    "target_col": main_target_col,
                    "target_cols": sample_keep_cols,
                    "target_col_chinese": "交通流量" if main_target_col == "traffic_volume" else str(main_target_col),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="生成Traffic数据集的中文文本描述（JSONL格式）。")
    parser.add_argument("--csv_path", type=str, required=True, help="输入CSV文件路径")
    parser.add_argument("--output_path", type=str, default=None, help="输出jsonl路径（默认：脚本目录下 Traffic_descriptions.jsonl）")
    parser.add_argument(
        "--window_lengths",
        type=int,
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="主目标列（仅单列，默认 traffic_volume）。不指定 --target_cols 时，会自动补充强相关列",
    )
    parser.add_argument(
        "--target_cols",
        type=str,
